...
├── VARIABLE_TYPES.MD          <- Reference guide for variable types used in this project
├── .env                       <- Environment variables (keep out of version control)
├── .gitignore
├── pyproject.toml             <- Project configuration and package metadata
├── requirements.txt           <- Requirements file, e.g. generated with `pip freeze > requirements.txt`
├── data
│   ├── interim                <- Intermediate data that has been transformed
│   │   ├── covid19_tweets_with_tone_copy.csv
│   │   ├── covid19_tweets_with_tone.csv
|   |   ├── covid19_tweets.csv
│   │   └── misclassified_texts.csv
